---
layout: page
title: 'AZTEC v1.3 Documentation'
...
* [Fleet Turnover](#fleet-turnover)
  ...
  * [Streamlined TCO for single vehicle](#streamlined-tco-for-single-vehicle)
  * [Retrofits](#retrofits)
* [Emissions](#emissions)
...
* [Sensitivity analysis](#sensitivity-analysis)
  ...
  * [Stochastic variable inputs](#stochastic-variable-inputs)
    * [Independent sweeping within a single input file](#independent-sweeping-of-multiple-variables-within-a-single-input-file) 
  ...
  * [Sensitivity module for multiple scenarios](#sensitivity-module-for-multiple-scenarios)
...
Version 1.2 was developed in 2026 by Stefan Walzer-Goldfeld and Gabe Hillman Alvarez.
...
(see [Required inputs](#required-inputs) and [Optional inputs](#optional-inputs)).
...
The variables below provide default values that are used in the emissions calculations unless specific inputs are provided by the user. 
The default inputs packaged in the model are intended to give baseline global coverage, but it is strongly encouraged to overwrite them with regionally-tailored values when possible.

To overwrite or add to the default inputs, just include the custom values as an input in the configuration file. For example, GHG emission intensities for a specific fuel can be added/overwritten by including the input `ghg_emission_rates = my_fuel_ghg_efs.csv` in the configuration file. 
To find relevant data to use as scenario inputs, please check the `example/inputs/` folder, as well as the [Model results repository](https://theicct.sharepoint.com/sites/ModelingCenter/Lists/Model%20results%20repository/AllItems.aspx) for inputs used in past AZTEC analyses.
...
| `transmission_loss`       | %       | Region, \[year\]                      | Electricity transmission and distribution losses (see [Transmission Loss](#transmission-and-distribution-losses)).                                                                                    |
...
### Streamlined TCO for single vehicle

Many features of AZTEC are designed to model complex fleet dynamics, but users may also use AZTEC to model the TCO of a single vehicle, 
e.g. to compare the TCO of a single battery electric truck against its ICE counterpart. 

This section provides a suggestion for how to specify such a setup, using an example in which the vehicle is purchased in 2024, operates through the end of 2029 (6-year service lifetime), and is then resold at the beginning of 2030.

- AZTEC infers the fleet composition in the configured `base_year` directly from the `stock` input; no sales are considered in the `base_year`. In this example, a vehicle purchase needs to occur in 2024, so you should set `base_year = 2023` and provide the `stock` input in "CY" 2023 with zero "Stock".
  To reiterate, the generic approach for this simple setup is to set `base_year` as the year before the purchases occur, and provide zero `stock` in that `base_year`.
- The end year should be chosen as the year of retirement, i.e. `base_year + 1 + <vehicle_service_lifetime>` . In this example, that means `end_year = 2030`.
- Use `growth_method = projected` and specify projections as 1 in the year of purchase (2024), 1 in the final year of operation (2029), and 0 in the year of retirement (2030). No need to provide any projections for 2023 because this is already specified in `stock`.
- Specify `purchase_method = None` so that sales are inferred from the projections. Then provide the `sales` input with powertrain shares in your purchase year (2024). This is the easiest input to use to distinguish between one scenario in which an ICE vehicle is purchased and another in which a BEV is purchased instead, e.g.:

  | Scenario        | Region  | Vehicle         | Technology | Control | Variable | 2024 |
  |-----------------|---------|-----------------|------------|---------|----------|------|
  | Baseline        | Seattle | class 8 drayage | diesel     | Euro VI | Share    | 1    |
  | Electrification | Seattle | class 8 drayage | electric   | BEV     | Share    | 1    |

- Specify `turnover_method = vehicle_service_lifetime`. In this example, the service lifetime is 6. 
- Define your `vehicle_component_costs` input starting with "MY" equal to the `base_year` (2023) to avoid data validation issues. Regardless, costs will only be incurred once purchases begin (`base_year + 1`, or 2024)
- Don't forget to define/rename the second scenario in your config file, e.g.:

  ```ini
  [Electrification]
  sales = fleet/sales.csv
  ```

With this setup, capital costs will be incurred in your `base_year + 1` (2024), operational expenses will be incurred through `end_year - 1` (2029), and resale value will be accrued in the `end_year` (2030).
Infrastructure inputs should be assembled similarly and their costs will follow a similar pattern.
...
To source component cost information for zero-emission HDVs, users may consult the [HD-ZEV Component Costs Database](https://theicct.sharepoint.com/:x:/s/HeavyDutyVehicles/Efncz1v3FElLkCWPGDqiFtoB4y2kZAkbbdQ_Epn0mOINDA?e=5OXab6) developed and maintained by Hussein Basma.
The database is updated quarterly and will be accompanied by a 'how to build an zero-emission truck' guide, to help users identify relevant components for bottom-up cost modeling.
...
### Blank input files devtool
AZTEC requires all inputs to be specified, even if there are no associated costs (e.g. revenue or age specific costs) 
or if some inputs may not be relevant for a specific study (e.g. emisison factors). 
If there are a large number of regions, routes, vehicles and technologies, creating the 
empty inputs can be tedious by hand. AZTEC has an accompanying `blank_file_generator.py` 
devtool that allows the user to create blank input files using the vehicle, technology, etc 
combinations from existing user input files, allowing users to more easily produce these empty inputs to allow for executable model runs. 
The following input files that are currently support are displayed 
in the table below, along with the additional input files used by the devtool.

| File                                   | Required input                    |
|----------------------------------------|-----------------------------------|
| `revenue`                              | `tco/vehicle_component_costs.csv` |
| `infrastructure_costs_age_specific`    | `tco/infrastructure_costs.csv`    |
| `vehicle_component_costs_age_specific` | `tco/vehicle_component_costs.csv` |
| `emission_factors`                     | `fleet/sales.csv`                           |

The devtool is called from the main AZTEC directory with the following command:

```commandline
python devtools/blank_file_generator.py <inputs_directory> <base_year>
```

The inputs directory must have the following file structure (see the `example`
folder in AZTEC for an example):
...
│   └── ...
└── tco/
    ├── vehicle_component_costs.csv
    ├── infrastructure_costs.csv
    └── ...
```

Note: the other input files that are used to develop complete, blank inputs,
like `vehicle_component_costs.csv` must be fully developed prior to running 
the devtool. Otherwise, the blank files outputted by this devtool will be incomplete.
...
See [Step-by-step sensitivity instructions](#step-by-step-sensitivity-instructions) or our [recorded training](https://theicct.sharepoint.com/:v:/s/ModelingCenter/ESz_ZCmYIkBMoqn9Pv2K5w8BcI6bjsjZ5COXEhhfrYXtuw?e=1WechQ&nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJTdHJlYW1XZWJBcHAiLCJyZWZlcnJhbFZpZXciOiJTaGFyZURpYWxvZy1MaW5rIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXcifX0%3D)
for more details.
...
| Scenario   | Region    | Technology | MY   | BatterySize | 
|------------|-----------|------------|------|-------------|
...
### Sensitivity analysis block diagram
...
![Sensitivity analysis flowchart for BEV](aztec/assets/sensitivity_flowchart_bev.png)
**Figure 1:** _Sensitivity analysis flowchart for BEVs._

![Sensitivity analysis flowchart for FCEV and H2ICE](aztec/assets/sensitivity_flowchart_h2.png)
...
#### Independent sweeping of multiple variables within a single input file

By default, all rows within the same stochastic variable input file are swept **together**: when one row is at its minimum, all other rows are also at their respective minimums. This is appropriate when the rows represent the same quantity at different time steps (e.g., electricity price in 2030 and 2035).

To vary rows **independently** within a single file — for example, to sweep electricity price and diesel price as separate dimensions — add an optional "SensGroup" column. Rows sharing the same "SensGroup" value are coupled (varied together), while rows with different "SensGroup" values are cross-joined to produce every possible combination. See `sensitivity/example/inputs/energy_costs_by_scen_swept.csv` for an example.
...
Otherwise, the sensitivity module will vary the cost of components directly.
...
- **Description:** BEV state variable input file for the most commonly available charging power (kW) and charging time (hr) by calendar year
- **Corresponding AZTEC input:** None
- **Example:** `sensitivity/example/inputs/battery_size.csv`
...
### Sensitivity module for multiple scenarios
The sensitivity module does not currently support running multiple scenarios in a single
pass. To run a sensitivity analysis across multiple scenarios, split it into one run per
scenario: filter the original scenario inputs down to a single scenario (e.g., `Baseline`)
and drop the scenario column before passing them to the model. This way the module does
not need to filter by sub-scenario names like
`Baseline~X` internally, which will cause data to not be picked up.
...
<button name='download' onclick="location.href='../AZTEC v1.3 Model Documentation.pdf'">Download as PDF</button>
